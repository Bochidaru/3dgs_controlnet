### !!! THIS IS TEMPORARY FILE, BECAUSE OF THE BIAS DATASET, DONT USE THIS IF WE HAVE NICE DS
...
class MyDataset(Dataset):
    def __init__(self, target_size=(512,512), isTest=False, use_cached_latent=False):
        ...
        with open(f'{self.root_path}/dataset.jsonl', 'rt') as f:
            for line in f:
                ...
                scene_tag = json_data.get("scene_tag", "")
                if scene_tag.startswith(("car4", "car5")):   ## tạm skip để đỡ bias data
                    continue
    ...
    def __getitem__(self, idx):
        ...
        if self.use_cached_latent:
            ...
            ref2_latent_path = get_save_path(ref1_path.split("/", 1)[1], self.cache_latent_root_path)
